...
object Cube:
  ...
  object Parser extends RegexParsers :
    ...
    def number: Parser[Num] = raw"-?\d+".r ^^ (_.toLong)

  extension (range: Range)
    def common(other: Range): Range =
  ...
  def leftRanges(rules: Vector[(State, Cuboid)]): Vector[Range] =
    rules
      ...
      .sliding(2)
      .flatMap {
        case Vector(a, b) => Seq(a to a, a + 1 to b - 1, b to b)
        case Vector(a) => Seq(a to a)
      }
      .distinct
      .filter(_.nonEmpty)
      .toVector
...
@main def day22(): Unit =
  ...
  val input = Source
    .fromURL(getClass.getResource("/22.txt"))
    .getLines()
